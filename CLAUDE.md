# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## Project Overview

This is an **Upptime-powered uptime monitoring** repository for CCCP.PS. Upptime uses GitHub Actions for scheduled health checks (every 5 minutes), GitHub Issues for incident tracking, and GitHub Pages for a public status dashboard at https://up.cccp.ps.

## Critical Rule: Only Edit `.upptimerc.yml`

The **only file that should be manually edited** is `.upptimerc.yml`. All other files are auto-generated:

- `.github/workflows/*.yml` — regenerated by the Setup CI workflow
- `api/` — generated uptime/response-time badge JSON
- `graphs/` — generated PNG charts
- `history/` — automated check logs and summary data
- `README.md` — auto-updated with status tables by Summary CI

Editing `.upptimerc.yml` triggers the Setup CI workflow, which regenerates workflows, templates, and redeploys the site.

## Architecture

```
.upptimerc.yml (config)
    │
    ├─→ Uptime CI (every 5 min) → checks endpoints → writes history/, creates issues on failure
    ├─→ Response Time CI (daily 23:00) → aggregates metrics → writes api/
    ├─→ Graphs CI (daily 00:00) → generates charts → writes graphs/
    ├─→ Summary CI (daily 00:00) → updates README.md
    ├─→ Static Site CI (daily 01:00) → builds Sapper site → deploys to gh-pages → up.cccp.ps
    ├─→ Setup CI (on .upptimerc.yml change) → regenerates all workflows
    └─→ Update Template CI (daily) → pulls latest Upptime version
```

## Required GitHub Secrets

- `GH_PAT` — Personal Access Token for git operations (optional, falls back to `github.token`)
- `SECRET_SITE_*` — monitored site URLs (e.g. `SECRET_SITE_001`, `SECRET_SITE_002`), referenced as `$SECRET_SITE_*` in config

## Commit Conventions

Automated commits use emoji prefixes and `[skip ci]` to prevent recursive CI:
- `🟩` / `🟥` — service status up/down
- `:bento:` — data updates (graphs)
- `:card_file_box:` — data updates (summary)
- `:pencil:` — README updates
- `:arrow_up:` — dependency updates

Manual commits use conventional prefixes: `style:`, `chore:`, `feat:`.

## Important Constraints

- Never force-push — breaks historical monitoring data
- Never manually edit workflow files — they get overwritten by Setup CI
- Upptime version is v1.41.0 — updated automatically by the update-template workflow
- The status site is built with Sapper (Svelte) and deployed via `peaceiris/actions-gh-pages`
